module RandomProxHypersExperiment
...
const CCU = MirrorDescentActorCritic.DefaultExperimentUtils
const CHECKPOINT_EVERY_DEFAULT = 50000000

include("./checkpoint.jl")

Reproduce.@generate_config_funcs begin
    "thresholds" => [-150, -200, -250, -350, -500]
    "eps_threshold_reached" => [25, 50, 100]
    "seed" => 7
    "steps" => 50000
    "env" => {
        # "type" => "CliffWorld",
    ...
    "agent" => {
        ...
        "λ_pow_max" => 5
        "λ_pow_base" => 10
        "num_md_updates_min" => 3
        "num_md_updates_max" => 250
        "device" => "cpu"
...
function main_experiment(config; progress=false, testing=false)
    ...
    @param_from thresholds config
    @param_from eps_threshold_reached config
    ...
    Reproduce.experiment_wrapper(config; testing=testing) do config
        ...
        with_logger(logger) do
            ...
            experiment_loop(
                agent, env, steps, checkpoint, checkpointer, thresholds,
                eps_threshold_reached;
...
function experiment_loop(
    agent, env, num_steps, checkpoint::Checkpoint, checkpointer::Checkpointer, thresholds,
    eps_threshold_reached;
    logger, logger_data, progress=false, testing=false,
)
    ...
    eps_returns = []
    thresholds_reached = Dict{Int,Any}()
    for ep in eps_threshold_reached
        thresholds_reached[ep] = Dict{Int,Any}(thresholds .=> -1)
    end
    ...
    while !done
        ...
        ep_reward, ep_steps = try
            ...
            if !(e isa DomainError || e isa ErrorException)
                throw(e)
            end
            done = true
            NaN32, 0
        ...
        for (thresh_eps, dict) in thresholds_reached
            for (thresh, v) ∈ dict
                if v > -1
                    # Already logged
                    continue
                elseif length(eps_returns) < thresh_eps
                    # Have not seen thresh_eps episodes yet
                    continue
                end
                if all(eps_returns[end-thresh_eps+1:end] .>= thresh)
                    thresholds_reached[thresh_eps][thresh] = eps
                end
            end
        end
        ...
        if done
            ...
            @data exp thresholds_reached=thresholds_reached
            return
        end
    end
end
...
function set_prox_md_hypers(seed, agent_config)
    rng = Xoshiro(seed)
    λ_exponent_min = agent_config["λ_pow_min"]
    λ_exponent_max = agent_config["λ_pow_max"]
    λ_pow_base = agent_config["λ_pow_base"]
    λ_exponent = (
        rand(rng, Float32) * (λ_exponent_max - λ_exponent_min) + λ_exponent_min
    )
    λ = λ_pow_base ^ λ_exponent
    agent_config["λ"] = λ

    @data exp λ=λ

    num_md_updates_min = agent_config["num_md_updates_min"]
    num_md_updates_max = agent_config["num_md_updates_max"]
    num_md_updates = rand(rng, num_md_updates_min:num_md_updates_max)
    agent_config["num_md_updates"] = num_md_updates

    @data exp num_md_updates=num_md_updates

    @show λ, num_md_updates
...
function construct_agent(::Type{BatchQAgent}, agent_config, env, seed)
    ...
    agent_config = set_prox_md_hypers(seed, agent_config)
    ...
    policy_f = CCU.construct_policy_approximator(policy, env, rng)
    ...
    q_f = CCU.construct_critic_approximator(value_fn, env, rng)
    ...
    device = if "device" in keys(agent_config)
...
function construct_env(config, seed)
    @param_from env config
    rng = Xoshiro(seed)
    return CCU.construct_env(env, rng)
end
