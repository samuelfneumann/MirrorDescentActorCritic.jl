...
function MirrorDescentActorCritic.envstep!(env::BraxEnv, action)
    obs = env(action)
...
function MirrorDescentActorCritic.action_space(env::BraxEnv)
...
function MirrorDescentActorCritic.observation_space(env::BraxEnv{Nothing})
    return env._observation_space
end

function MirrorDescentActorCritic.start!(env::BraxEnv)
    if env._collect
        env._collect = false
...
function MirrorDescentActorCritic.reward(env::BraxEnv)
    return (env._state.reward |> from_jax |> cpu_device())[]
end

MirrorDescentActorCritic.isterminal(env::BraxEnv) = env._state.done |> from_jax
function MirrorDescentActorCritic.isterminal(env::BraxEnv{Nothing})
    return (env._state.done |> from_jax |> cpu_device())[] |> Bool
end

MirrorDescentActorCritic.γ(env::BraxEnv) = isterminal(env) ? 0f0 : env._γ
