...
MirrorDescentActorCritic.action_space(g::GymnasiumEnv) = g.action_space
MirrorDescentActorCritic.observation_space(g::GymnasiumEnv) = g.observation_space
MirrorDescentActorCritic.reward(g::GymnasiumEnv) = g.reward
MirrorDescentActorCritic.isterminal(g::GymnasiumEnv) = g.terminal
MirrorDescentActorCritic.γ(g::GymnasiumEnv) = isterminal(g) ? 0f0 : g.γ
...
function MirrorDescentActorCritic.envstep!(env::GymnasiumEnv, action)
...
function MirrorDescentActorCritic.start!(
    env::GymnasiumEnv{T};
...
function MirrorDescentActorCritic.render(env::GymnasiumEnv)
    if pyis(env.pyenv, pybuiltins.None)
...
function MirrorDescentActorCritic.stop!(env::GymnasiumEnv)
    if pyis(env.pyenv, pybuiltins.None)
        return
    end
    env.pyenv.close()
