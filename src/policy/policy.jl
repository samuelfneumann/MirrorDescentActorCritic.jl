...
const _fEPSILON = 1f-3 #1f-3/1f-4 worked before, but they're really aggressive
...
macro _delegate_extended_distributions(D, pre)
    ...
    return quote
        $MirrorDescentActorCritic.logprob_function(::IIDPolicy{V,$D}) where {V} = $logpdf
        $MirrorDescentActorCritic.kl_function(::IIDPolicy{V,$D}) where {V} = $kldiv
        $MirrorDescentActorCritic.quantile_function(::IIDPolicy{V,$D}) where {V} = $quantile
        $MirrorDescentActorCritic.cdf_function(::IIDPolicy{V,$D}) where {V} = $cdf
        $MirrorDescentActorCritic.ccdf_function(::IIDPolicy{V,$D}) where {V} = $ccdf
    end
end
...
include("epsilon_greedy_policy.jl")
