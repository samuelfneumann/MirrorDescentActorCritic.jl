...
"""
    logprob(
        ::AbstractParameterisedPolicy, f, ps, st, states::AbstractArray,
        [actions::AbstractArray],
    )
    logprob(
        ::IIDPolicy, f, ps, st, states::AbstractArray,
        actions::AbstractArray,
    )

Return the log probability of actions in states.

TODO: mention dimensions of input states, actions etc.
"""
# function logprob(p::AbstractParameterisedPolicy, f, ps, st, states, actions)
#     error("logprob not implemented for policy $p")
# end

# function logprob(p::AbstractParameterisedPolicy, f, ps, st, states)
#     error("logprob not implemented for policy $p")
# end
...
continuous(::AbstractValuePolicy) = false
discrete(::AbstractValuePolicy) = true

"""
    sample(::AbstractValuePolicy, rng::AbstractRNG, qs)

Sample a single action from the policy for action values `qs`.
"""
function sample(::AbstractValuePolicy, rng::AbstractRNG, qs)
    error("sample not implemented for states of type $(typeof(qs))")
end

function sample(p::AbstractValuePolicy, qs)
    return sample(p, Xoshiro(), f, qs; num_samples=num_samples)
end

"""
    prob(p::AbstractValuePolicy, qs)

Calculate the probabilities of taking each action, where each action has value `qs`. This
can be useful for off-policy learning with importance sampling.
"""
function prob(p::AbstractValuePolicy, qs)
    error("prob not implemented for states of type $(typeof(states))")
end
