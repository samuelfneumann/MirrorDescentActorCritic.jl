...
function construct_env(config, rng)
    ...
    type = getproperty(MirrorDescentActorCritic, Symbol(type))
...
function construct_env(type::ClassicControlEnvironment, config, rng)
    @param_from kwargs config
    ks = keys(kwargs)
    vs = values(kwargs)
    keywords = [Symbol(k) => v for (k, v) in zip(Symbol.(ks), vs)]

    @param_from continuous kwargs
    env = if continuous
...
function construct_policy_approximator(
    ...
    in_size = if feature_creator != nothing
        ...
        size(MirrorDescentActorCritic.observation_space(env), 1)
    end

    out_size = if MirrorDescentActorCritic.continuous(MirrorDescentActorCritic.action_space(env))
        size(MirrorDescentActorCritic.action_space(env), 1)
    else
        MirrorDescentActorCritic.action_space(env).n[1]
    end
...
function final_act(
    ...
    # TODO: Probably better to use exp(LOG_STD_THRESHOLD * tanh(x)) or softplus(x) *
    # STD_THRESHOLD
    action_scale = MirrorDescentActorCritic.high(action_space(env))[1]
    @assert (
        MirrorDescentActorCritic.high(action_space(env))[1] ==
        abs(MirrorDescentActorCritic.low(action_space(env))[1])
    )
    return let LOG_STD_THRESHOLD = LOG_STD_THRESHOLD
        (
...
function get_critic_outputs(::Val{:Q}, env)
    return if MirrorDescentActorCritic.continuous(MirrorDescentActorCritic.action_space(env))
        1
    else
        MirrorDescentActorCritic.action_space(env).n[1]
    end
...
get_critic_inputs(::Val{:V}, env) = size(MirrorDescentActorCritic.observation_space(env))[1]
function get_critic_inputs(::Val{:Q}, env)
    return if MirrorDescentActorCritic.continuous(MirrorDescentActorCritic.action_space(env))
        env_in = size(MirrorDescentActorCritic.observation_space(env))[1]
        action_size = size(MirrorDescentActorCritic.action_space(env))[1]
        env_in + action_size
    else
        size(MirrorDescentActorCritic.observation_space(env))[1]
    end
...
function construct_critic_approximator(
    ...
    if MirrorDescentActorCritic.continuous(MirrorDescentActorCritic.action_space(env))
        if n > 1
            return Chain(
                MirrorDescentActorCritic.Concat(1),
                Parallel(
                    (args...) -> cat(args...; dims=ndims(args[1]) + 1),
                    paths...
                ),
            ...
            return Chain(MirrorDescentActorCritic.Concat(1), paths[1])
        end
    else
        if n > 1
            return Chain(
