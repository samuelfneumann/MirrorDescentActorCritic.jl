...
function construct_policy_approximator(::Val{:Lux}, policy::Symbol, config, env, rng)
    if MirrorDescentActorCritic.continuous(MirrorDescentActorCritic.action_space(env))
        in_size = size(MirrorDescentActorCritic.observation_space(env))[1]
        out_size = size(MirrorDescentActorCritic.action_space(env))[1]
    else
        in_size = size(MirrorDescentActorCritic.observation_space(env))[1]
        out_size = MirrorDescentActorCritic.action_space(env).n[1]
    end
