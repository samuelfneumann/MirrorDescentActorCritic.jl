...
function construct_critic(type::Val{:DiscreteQ}, env, reduct)
    return MirrorDescentActorCritic.DiscreteQ(reduct)
end
...
function construct_critic(type::Val{:Q}, env, reduct)
    if MirrorDescentActorCritic.continuous(MirrorDescentActorCritic.action_space(env))
        return MirrorDescentActorCritic.Q(reduct)
    else
        return MirrorDescentActorCritic.DiscreteQ(reduct)
    end
end
...
function construct_critic(type::Val{:V}, env, reduct)
    return MirrorDescentActorCritic.V(reduct)
end
...
get_policy_config(::Val{:εGreedyPolicy}) = OneParamPolicyConfig()
...
function construct_policy(config, env)
    ...
    constr = getproperty(MirrorDescentActorCritic, policy_type)
...
function get_optimiser(opt_iden, config)
    if opt_iden isa Nothing || opt_iden == "nothing"
        # Sometimes we don't want an optimiser. This is signaled as Nothing
        return nothing
    end
...
function get_update(config)
    @param_from type config
    update = getproperty(MirrorDescentActorCritic, Symbol(type))
...
function get_update(update_type::Type{DualAveragingMPI}, config)
    try
        @param_from τ config
        @param_from λ config
        if "offset" in keys(config)
            @param_from offset config
            return update_type(τ, λ; ensure_stochastic=true, offset=offset)
        elseif "ensure_stochastic" in keys(config)
            @param_from ensure_stochastic config
            return update_type(τ, λ; ensure_stochastic=ensure_stochastic)
        else
            return update_type(τ, λ)
        end

    catch e
        if e isa UndefVarError
            error("$(update_type) needs: τ (float) and λ (float): $e")
        else
            throw(e)
        end
    end
...
function construct_buffer(::Val{:ExperienceReplay}, env, config)
    ...
    return MirrorDescentActorCritic.ExperienceReplay(env, capacity)
end

function construct_buffer(::Val{:VectorExperienceReplay}, env, config)
    @param_from capacity config
    return MirrorDescentActorCritic.VectorExperienceReplay(env, capacity)
end
...
function construct_buffer(::Val{:GAEBuffer}, env, config)
    ...
    @param_from γ config
    @param_from λ config
    return MirrorDescentActorCritic.GAEBuffer(env, capacity, γ, λ)
end
