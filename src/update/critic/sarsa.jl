...
function setup(
    ...
    # Initialize the Bellman regularizers
    reg_st = [
        setup(reg, π, qf, qf_f, qf_θ, qf_st, Lux.replicate(rng))
        for reg in up._reg
    ]
...
function update(
    ...
    @warn "Sarsa with AbstractValuePolicy's has not been tested" maxlog=1
    up = st._update
    rng = Lux.replicate(st._state.rng)

    # Compute the target using the target network
    q_tp1, qf_target_st = predict(qf, qf_f, qf_target_θ, qf_target_st, s_tp1)
    a_tp1 = sample(π, rng, q_tp1)
    ...
    target = if length(up._reg) == 0
        ...
        # Accumulate the regularization to add to the bellman operator
        ####################################################################
        accum_reg_term, qf_st, reg_st = regularise(
            st._state.reg_st[1], s_tp1, π, qf, qf_f, qf_θ, qf_st,
        )
        reg_states = BellmanRegulariserState[reg_st]

        for i in 2:length(st._state.reg_st)
            reg_term, qf_st, reg_st = regularise(
                st._state.reg_st[i], s_tp1, π, qf, qf_f, qf_θ, qf_st,
            )
            accum_reg_term .+= reg_term
            push!(reg_states, reg_st)
        end
        ...
        r_tp1 .+ γ_tp1 .* (q_tp1[a_tp1] .+ accum_reg_term)
   end
