...
function regularise(
    ...
    qs, qf_st = predict(qf, qf_f, qf_θ, qf_st, states)
    lnπ = logprob(π, qs)
    entropy = -mean(lnπ; dims=1) # Compute ℍ(π) = 𝔼_π[ln(π(a ∣ s))]
    entropy = dropdims(entropy; dims=1)

    return (reg._τ * entropy), qf_st, RegulariserState(reg, NamedTuple())
end
####################################################################
